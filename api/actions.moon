...
--
-- This is not a function but an associative table indexed by WSL action names. It contains functions performing the corresponding actions. Using these functions is similar to calling #wesnoth.fire, while setting entries of the table is similar to calling #wesnoth.register_wsl_action.
-- Note: When calling an action handler directly through its function stored in wesnoth.wsl_actions, the engine is not involved. As a consequence, whether variable substitution will happen is up to the handler. In particular, if the argument is a plain table, the caller should have substituted WSL variables beforehand to be on the safe side. Moreover, table arguments might be modified by the action handler, so they should usually not be reused for consecutive calls. If variable substitution should happen and/or table content should be preserved, one can call #wesnoth.tovconfig and pass its result to the handler. Calling #wesnoth.fire is another possibility.
-- @table wesnoth.wsl_actions
-- @within actionsAndEvents
-- @usage wesnoth.wsl_actions.freeze_unit = (cfg) ->
...
--
-- This is an associative table like wesnoth.wsl_actions. You can use it to define new conditional wsl tags that will be recognized in WSL when using [if], [show_if], [while], etc., or more generally when wesnoth.eval_conditional is run.
-- You cannot override the meaning of any core conditional tags.
-- @table wesnoth.wsl_conditionals
-- @usage function wesnoth.wsl_conditionals.foo(cfg)
...
-- @func on_event: function called before each WSL event is executed; its argument is the event name; other event arguments can be recovered from wesnoth.current.event_context.
-- some value that survives save/load cycles, but that is not forwarded to the next level
-- @usage level_local_data = 0
-- old_on_load = wesnoth.game_event.on_load
-- wesnoth.game_event.on_load = (cfg) ->
--     for i = 1,#cfg
--         if cfg[i][1] == "my_data"
...
-- disallow_undo = () ->
--	wesnoth.wsl_actions.event { name: wesnoth.current.event_context.name }
...
-- @usage wesnoth.fire_event = (event_name, [x1, y1, [x2, y2]], [first_weapon, [second_weapon]]) ->
fire_event = (event_name, x1, y1, x2, y2, first_weapon, second_weapon) =>
    ...
    execute_event_handler = (handler, primary, second, first, second, ENV) ->
        ...
        -- return false, "failed side filter" if handler.filter_side and not wesnoth.match_side(ENV.side_number, handler.filter_side)
        -- return false, "failed condition" if handler.filter_condition and not wesnoth.eval_conditional(handler.filter_condition)
    ...
    if x1
        try
            ...
            catch: (err) -> error "wesnoth.fire_event: #{err}"
    if x2
...
remove_event_handler = (id) ->
    for name, handlers in pairs wesnoth.game_events
...
-- wesnoth.wsl_actions.clear_variable { name: "to_be_deleted" }               -- correct
-- wesnoth.wsl_actions.clear_variable { name: "$varname" }                    -- error: try to delete a variable literally called "$varname"
-- wesnoth.wsl_actions.clear_variable(wesnoth.tovconfig { name: "$varname" }) -- correct: "$varname" is replaced by "to_be_deleted" at the right time


----
-- @function helper.set_wsl_action_metatable
set_wsl_action_metatable = () ->
-- Sets the metatable of a table so that it can be used to fire WSL actions. Returns the table. The fields of the table are then simple wrappers around a call to #wesnoth.fire.
