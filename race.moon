...
markov_generate_name = (prefixes, chain_size, max_len) ->

    if prefixes.empty() or chain_size == 0
        return "" -- return ucs4::string();

    res = "" -- ucs4::string prefix, res;
    prefix = ""
    ...
    random = {} -- std::vector<int> random(max_len);

    for j = 0, max_len -- size_t j = 0;
        random[j] = next_random!

    j = 0
    while res\size! < max_len

        -- const markov_prefix_map::const_iterator i = prefixes.find(prefix);
        -- if(i == prefixes.end() || i->second.empty()) { return res; }
        return res if prefixes[prefix]

        -- c = i-> -- const ucs4::char_t c = i->second[random[j++]%i->second.size()];

        return res if c == 0 -- if(c == 0) { return res; }

        -- res.resize(res.size()+1);
        -- res[res.size()-1] = c;
        -- prefix.resize(prefix.size()+1);
        -- prefix[prefix.size()-1] = c;
        -- while(prefix.size() > chain_size) { prefix.erase(prefix.begin()); }
    ...
    originalRes = res -- ucs4::string originalRes = res;
    while not res\empty!
        prefixLen = if chain_size < res.size! then chain_size else res.size! -- const int prefixLen = chain_size < res.size() ? chain_size : res.size();
        prefix = hmmm    -- prefix = ucs4::string(res.end() - prefixLen, res.end());
        ...
        --        return res;
        -- }
        -- // The current ending is invalid, remove the last character and retry.
        -- res.pop_back();
...
generate_name = (race, gender) -> -- std::string unit_race::generate_name(unit_race::GENDER gender) const
    -- return unicode_cast<utf8::string>(markov_generate_name(next_[gender], chain_size_, 12));
    return markov_generate_name(gender, chain_size, 12)
